...
class NVENCException : public std::exception {
 public:
  ...
  static NVENCException makeNVENCException(
      const std::string& errorStr, const NVENCSTATUS errorCode, const std::string& functionName,
...
class NvEncoder {
 ...
 public:
   ...
   */
  static void GetChromaSubPlaneOffsets(
 ...
 protected:
  ...
  NvEncoder(
      ...
      NV_ENC_BUFFER_FORMAT eBufferFormat, uint32_t nOutputDelay, bool bMotionEstimationOnly,
  ...
  void RegisterInputResources(
      ...
      NV_ENC_BUFFER_FORMAT bufferFormat, bool bReferenceFrame = false);
  ...
  NV_ENC_REGISTERED_PTR RegisterResource(
      ...
      NV_ENC_BUFFER_FORMAT bufferFormat, NV_ENC_BUFFER_USAGE bufferUsage = NV_ENC_INPUT_IMAGE);
  ...
  void* GetCompletionEvent(uint32_t eventIdx) {
    return (m_vpCompletionEvent.size() == (size_t)m_nEncoderBuffer) ? m_vpCompletionEvent[eventIdx] : nullptr;
  }
   ...
   */
  NVENCSTATUS DoMotionEstimation(
