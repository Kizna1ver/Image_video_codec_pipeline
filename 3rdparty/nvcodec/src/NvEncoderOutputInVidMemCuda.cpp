  ...
  for (uint32_t i = 0; i < m_pOutputBuffers.size(); ++i) {
    if (m_pOutputBuffers[i]) {
      NV_ENC_REGISTERED_PTR registeredPtr = RegisterResource(
          (void *)m_pOutputBuffers[i], NV_ENC_INPUT_RESOURCE_TYPE_CUDADEVICEPTR, bfrSize, 1, bfrSize,
...
void NvEncoderOutputInVidMemCuda::EncodeFrame(
    std::vector<NV_ENC_OUTPUT_PTR> &pOutputBuffer, NV_ENC_PIC_PARAMS *pPicParams) {
  pOutputBuffer.clear();
...
void NvEncoderOutputInVidMemCuda::RunMotionEstimation(std::vector<NV_ENC_OUTPUT_PTR> &pOutputBuffer) {
  ...
  NVENCSTATUS nvStatus =
      DoMotionEstimation(m_vMappedInputBuffers[bfrIdx], m_vMappedRefBuffers[bfrIdx], m_vMappedOutputBuffers[bfrIdx]);
