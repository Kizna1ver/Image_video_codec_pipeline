...
NvDecoder::NvDecoder(
    CUcontext cuContext, bool bUseDeviceFrame, cudaVideoCodec eCodec, bool bLowLatency, bool bDeviceFramePitched,
...
NvDecoder::~NvDecoder()
{
  ...
  cuCtxDestroy(m_cuContext); // 完全销毁上下文,似乎和NvDoder的析构有冲突，corrupted size vs. prev_size，但是不销毁又会泄漏显存。
